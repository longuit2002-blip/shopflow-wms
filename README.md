...
[![Stage](https://img.shields.io/badge/stage-Sprint--1--redux%20%E2%9C%85-brightgreen)](docs/phase-gates/2026-05-12-sprint-1-redux-signoff.md)
[![License](https://img.shields.io/badge/license-TBD-lightgrey)](#license)

**Current stage**: **Phase-1 Sprint-1-redux complete (2026-05-12)** — tagged `v0.3.0-sprint-1-redux`. The reservation ledger ships against the DB-per-tenant foundation. `ReservationRepository` runs the conditional-CTE INSERT at READ COMMITTED (the v3.0 correction over the v2.0 SERIALIZABLE shape) with row-lock contention serialization on `stock_items` + `UNIQUE(order_id)` idempotency. Full `IReservationRepository` surface: `TryReserveAsync` / `FindByOrderIdAsync` / `ConfirmAsync` / `ReleaseAsync` / `ReleaseExpiredAsync`. Multiplexed `ReservationExpiryWorker` fans out across `Ready` tenants per tick. 14 Inventory integration tests + 5 FsCheck properties + the W3 5×1000 noisy-neighbor scale gate. See [Sprint-1-redux sign-off](docs/phase-gates/2026-05-12-sprint-1-redux-signoff.md) for measured numbers + deferred items, and [docs/solutions/2026-05-12-readcommitted-conditional-cte-correctness.md](docs/solutions/2026-05-12-readcommitted-conditional-cte-correctness.md) for the ReadCommitted decision rationale. **Previous**: [Phase-0-redux sign-off](docs/phase-gates/2026-05-12-phase-0-redux-signoff.md) (`v0.2.0-phase-0-redux`). **Next**: Sprint-2-redux (Inbound module W4) — plan TBD.
