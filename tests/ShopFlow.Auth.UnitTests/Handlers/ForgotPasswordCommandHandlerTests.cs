...
public sealed class ForgotPasswordCommandHandlerTests
{
    ...
    private ForgotPasswordCommandHandler BuildHandler() => new(
        _users, _resetTokens, _hasher, _outbox, _auditLog,
        NullLogger<ForgotPasswordCommandHandler>.Instance,
        _requestContext, _clock, Options.Create(_options));
    ...
    public async Task KnownActiveUser_EmitsResetTokenAndAudit()
    {
        ...
        _resetTokens.GetLastIssuedAtAsync(user.Id, Arg.Any<CancellationToken>())
            .Returns(Task.FromResult<DateTime?>(null));
        _resetTokens.AddAsync(Arg.Any<byte[]>(), user.Id, Arg.Any<DateTime>(), Arg.Any<CancellationToken>())
