...
public sealed class VerifyEnrollMfaCommandHandlerTests
{
    ...
    private readonly IEnrollmentSecretStore _enrollmentStore = Substitute.For<IEnrollmentSecretStore>();
    ...
    private VerifyEnrollMfaCommandHandler BuildHandler() => new(
        _users, _codec, _enrollmentStore, _totp, _cipher, _secrets, _recoveryCodes,
        _hasher, _issuer, _refreshStore, _outbox, _auditLog,
        NullLogger<VerifyEnrollMfaCommandHandler>.Instance, _requestContext, _clock);

    [Fact]
    public async Task InvalidEnrollmentToken_ReturnsInvalidCredentials_NoAuditRow()
    {
        _codec.TryDecode(Arg.Any<string>(), Arg.Any<DateTime>())
    ...
    public async Task Happy_EnrollsAndEmitsMfaEnrolledAudit()
    {
        ...
        _codec.TryDecode(Arg.Any<string>(), Arg.Any<DateTime>())
            .Returns(new MfaChallengePayload(
                UserId: user.Id,
                TenantSlug: "t1",
                RememberMe: false,
        ...
        _enrollmentStore.ConsumeAsync("t1", user.Id, enrollmentId, Arg.Any<CancellationToken>())
            .Returns(Task.FromResult<byte[]?>(rawSecret));
        _totp.VerifyOtp(rawSecret, "123456", _clock)
            .Returns(new OtpVerificationResult(IsValid: true, TimeStep: 12345L));
        _cipher.Encrypt(rawSecret, Arg.Any<Guid>(), user.Id)
            .Returns((new byte[64], 1));
        _hasher.Hash(Arg.Any<string>(), Argon2Profile.RecoveryCode)
            .Returns(ValidHash);
        _recoveryCodes.AddBatchAsync(user.Id, Arg.Any<IReadOnlyList<string>>(), Arg.Any<CancellationToken>())
        ...
        _refreshStore.IssueAsync("t1", user.Id, false, Arg.Any<CancellationToken>())
