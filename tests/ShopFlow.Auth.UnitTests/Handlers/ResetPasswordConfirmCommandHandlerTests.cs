...
public sealed class ResetPasswordConfirmCommandHandlerTests
{
    ...
    private ResetPasswordConfirmCommandHandler BuildHandler() => new(
        _resetTokens, _users, _hasher, _refreshStore, _auditLog,
        NullLogger<ResetPasswordConfirmCommandHandler>.Instance, _clock);
    ...
    public async Task Happy_ConsumesTokenRotatesAndEmitsAudit()
    {
        ...
        _resetTokens.TryConsumeAsync(Arg.Any<byte[]>(), _clock, Arg.Any<CancellationToken>())
        ...
        var result = await BuildHandler().Handle(Cmd("rawtoken", "newPassword1"), CancellationToken.None);
