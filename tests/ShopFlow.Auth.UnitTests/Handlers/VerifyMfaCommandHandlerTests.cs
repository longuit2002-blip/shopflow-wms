...
public sealed class VerifyMfaCommandHandlerTests
{
    ...
    private VerifyMfaCommandHandler BuildHandler() => new(
        _codec, _users, _secrets, _totp, _cipher, _recoveryCodes, _hasher,
        _issuer, _refreshStore, _auditLog,
        NullLogger<VerifyMfaCommandHandler>.Instance, _requestContext, _clock);

    private static VerifyMfaCommand Cmd(string challenge, string? otp = "123456", string? recovery = null) =>
    ...
    public async Task Happy_OtpVerified_EmitsMfaUsedAudit()
    {
        ...
        _secrets.GetAsync(user.Id, Arg.Any<CancellationToken>())
            .Returns(Task.FromResult<TotpSecretView?>(view));
        _cipher.Decrypt(view.EncryptedSecret, view.KeyId, Arg.Any<Guid>(), user.Id)
            .Returns(new byte[32]);
        _totp.VerifyOtp(Arg.Any<byte[]>(), "123456", _clock)
