...
ADR-0003 (database-per-tenant for compliance hard isolation) was accepted on 2026-05-11. **ADR-0001's decision stands**: Aspire AppHost remains the source of truth for local development, hand-maintained Docker Compose remains the production manifest. The pivot does not change the orchestrator choice.

What changes inside the AppHost configuration:

- A `pgbouncer` resource is added alongside the `postgres` resource. PgBouncer is non-optional from Phase-0-redux per ADR-0003 (transaction-pooling mode is the only sustainable shape with 25-50 tenant DBs).
- The control-plane database (`shopflow_control`) is created on AppHost startup before any service starts.
- Two dev tenants (`dev1`, `dev2`) are provisioned on AppHost startup via `shopflow-migrate provision`. Their existence is what keeps multi-tenant code paths exercised every working day.
- The Compose production manifest gains a corresponding `pgbouncer` service entry; CI's "both manifests describe the same stack" smoke check is updated to include it.

The < 60s cold-start target is reviewed: provisioning two dev tenants adds ~30-60s on cold start (CREATE DATABASE × 2 + migrations × 2 × N modules). Phase-0-redux benchmarks the actual number; if it pushes past < 90s, the decision is to ship with a slightly relaxed target rather than skip dev-tenant provisioning, because skipping would defeat the multi-tenant-every-day discipline.

**See also**: ADR-0003, `docs/redesign/02-technical-design-document.md` §1 (multi-tenancy), §2 (provisioning), §16.1 (dev mode).
