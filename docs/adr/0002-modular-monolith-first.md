...
## Postscript — 2026-05-11 — multi-tenancy pivot impact

ADR-0003 (database-per-tenant for compliance hard isolation) was accepted on 2026-05-11. **ADR-0002's central decision stands**: bootstrap as a modular monolith, mechanical 6-process split planned for W6. Module split timing is unchanged — the pivot is orthogonal to the process-split timing. Both can land independently.

What is superseded:

- The original Decision section bullet "_All cross-cutting discipline (`tenant_id` on every row, RLS policies, outbox interceptor, correlation-context propagation, Result<T> error handling, Roslyn-analyzer-enforced canon) is wired from the first commit_" is amended by ADR-0003. The cross-cutting discipline still applies, but with concrete shape changes:
  - **No `tenant_id` on every row** — the database itself is the tenant boundary.
  - **No RLS policies** — they were the enforcement mechanism for row-level tenancy; meaningless in DB-per-tenant.
  - **`TenancyInterceptor` is removed** — its job (stamping tenant_id, refusing cross-tenant writes) does not exist when each tenant has its own DB.
  - **Routing middleware** + per-request DbContext factory replace the interceptor as the new tenant-correctness primitive.
  - **Outbox interceptor** stays in shape but writes to the tenant's own outbox table.
  - **Correlation-context propagation, Result<T> error handling, Roslyn analyzers** stay; analyzers are re-derived for the new shape (see `docs/redesign/02-technical-design-document.md` §18).

What is also superseded:

- Implicit-but-load-bearing claim from `02-technical-design-document.md.docx` v2.0 §4.5 ("the cheapest scale decision in the whole design: zero runtime cost, eliminates the most painful migration") — this was the framing that locked in RLS as the multi-tenancy approach. ADR-0003 documents why that framing was wrong against a compliance-driven driver. The "painful migration" is now the redesign itself.
