    ...
    where TRequest : notnull
{
    ...
    )
    {
        ...
        if (activity is not null)
        {
            try
            {
                activity.SetTag("tenant.id", _requestContext.TenantId.ToString());
                activity.SetTag("tenant.slug", _requestContext.TenantSlug);
            }
            catch (InvalidOperationException)
            {
                // Background / message-driven paths can reach this behavior
                // before the request context has been bound (e.g. some
                // consumer middleware ordering). Tag the span as unset
                // rather than throwing.
                activity.SetTag("tenant.id", "(unset)");
            }
            ...
            {
                activity.SetTag("shopflow.correlation_id", "(unset)");
            }
        }
